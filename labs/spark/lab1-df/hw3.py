...
if __name__ == "__main__":
    ...
    spark = (SparkSession.builder.appName('hm3').enableHiveSupport().getOrCreate())
    print("reading data...")
    ### reading row data
    ...
    filtered.coalesce(1).write.mode('overwrite').format("csv").option("header", "true").save("gs://bigdata_procamp/labs/lab_3/most_popular_dest.tsv")
    ### output overall statistics for debugging
    ranked.coalesce(1).write.mode('overwrite').format("csv").option("header", "true").save("gs://bigdata_procamp/labs/lab_3/ranked_dest.tsv")
    ...
    aggregated.select("AIRLINE", "ORIGIN_AIRPORT", "canceled_percentage").filter(aggregated["ORIGIN_AIRPORT"] != waco_code).orderBy("AIRLINE", 'canceled_percentage').\
            coalesce(1).write.mode('overwrite').json('gs://bigdata_procamp/labs/lab_3/canceled_flights.json')
            
    ### output statistics for waco       
    aggregated.filter(aggregated["ORIGIN_AIRPORT"] == waco_code).orderBy("AIRLINE", 'canceled_percentage').\
            coalesce(1).write.mode('overwrite').format("csv").option("header", "true").save("gs://bigdata_procamp/labs/lab_3/waco_canceled_flights.csv")  
